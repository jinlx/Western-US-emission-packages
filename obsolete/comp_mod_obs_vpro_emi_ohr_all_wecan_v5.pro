...
@GCRateConst_c3h8
@GCRateConst
pro comp_mod_obs_vpro_emi_ohr_all_wecan_v5,$
        plume=plume,$
        filter1_other=filter1_other,$
        filter2_nobb=filter2_nobb,filter2_bb=filter2_bb,$
        filter3_nobb=filter3_nobb,filter3_bb=filter3_bb,$
        filter4_nobb=filter4_nobb,filter4_bb=filter4_bb,$
        filter5_nobb=filter5_nobb,$
        young=young,intermed=intermed,GT4=GT4,LT4=LT4, aged=aged, emipass = emipass,nonsmoke=nonsmoke,$
        boise=boise,CentralValley = CentralValley, $
        all=all, obs_only=obs_only,each=each,onebyone=onebyone,addup=addup, chemistry=chemistry,newVOCs=newVOCs ,$
        ind_co=ind_co,ind_benz=ind_benz,ind_o3=ind_o3,ind_pan=ind_pan,ind_no2=ind_no2, $;; it should be used with 'each' keyword_set
        OVOCs = OVOCs, NMHCs = NMHCs, $
        test=test,cloud1=cloud1,cloud2=cloud2,$ 
        n_criteria=n_criteria,$
        nested=nested,fbf=fbf,save=save,$
        metrics=metrics, $
        prs=prs, $
        ...
        tempplot=tempplot,presplot=presplot,ndplot=ndplot, OHrplot=OHrplot, $ ; OHr dependes on mixing ratio, koh (temperature), number density(prs and temperature), and OH reacitivity
        OHr_output=OHr_output, $
        bkg_corr = bkg_corr, codeployed=codeployed, obs_major=obs_major
...
;;need to changed later!!!!!!!!!!!!!!!!!!!!!!!
    ...
    ;; prepare data for 20180826, lat is less than zero and it is not simulated by the obs,fixed
            ...
            ;; PTR    
                    ...
                    tmp_ch3cn_obs_ptr = tmp_ch3cn_obs_ptr[ind_enter_time[0]:ind_exit_time[-1]]; we also have mixing ratio of ch3cn, need to check the unit lixu
            ...
            ; TOGA
                    ...
                    tmp_hcn_obs_toga = tmp_hcn_obs_toga[ind_enter_time[0]:ind_exit_time[-1]]
...
;; ============================
    ...
    ;; concert hhmm(utc) into ss(utc)      
            ...
            ;        ch3oh_gc= [ch3oh_gc, gc.MOH*1e9]
                    ...
                    ;;temperature
                    tmp_temperature_gc_gfas = tmp_temperature_gc_gfas[ind_enter_time[0]:ind_exit_time[-1]]
             ;; add vocs
...
;; ========keep the same for regression, profiles and time series=================
        ...
        if keyword_set(obs_only) then rows = 3
        if keyword_set(obs_only) then cols = 3

        if keyword_set(NMHCs) then rows = 2
        if keyword_set(NMHCs) then cols = 2
        
        if keyword_set(newVOCs) then rows = 3
        if keyword_set(newVOCs) then cols = 1
        ...
        if keyword_set(newVOCs) then s_end = 2
            ...
            if keyword_set(obs_only) then begin
                case s of                     
                    ...
                    0:begin
                        ...
                        gvoc_gfas_bkg_corr = hcho_gc_gfas + 0.35
                    ...
                    1:begin
                        ...
                        gvoc_gfas_bkg_corr = ald2_gc_gfas 
                    ...
                    2:begin
                        ...
                        gvoc_gfas_bkg_corr = acet_gc_gfas 
                    ...
                    3:begin
                        ...
                        gvoc_gfas_bkg_corr = hcooh_gc_gfas 
                    ...
                    4:begin
                        ...
                        gvoc_gfas_bkg_corr = mek_gc_gfas 
                    ...
                    6:begin
                        ...
                        gvoc_gfas_bkg_corr = benz_gc_gfas 
                    ...
                    7:begin
                        ...
                        gvoc_gfas_bkg_corr = c7h8_gc_gfas 
                    ...
                    8:begin
                        ...
                        gvoc_gfas_bkg_corr = c8h10_gc_gfas 
    ...
    ;;set for individual species, e.g., CO, benzene
            if keyword_set(each) then begin
                if keyword_set(ind_co) then begin
                    ...
                    gvoc_gfas_bkg_corr = co_gc_gfas+10
            ...
            if keyword_set(all) then begin
                case s of                     
                    ...
                    0:begin
                        ...
                        xrange = [0,6]
                        if keyword_set(boise) then xrange = [0,6]
                        if keyword_set(CentralValley) then xrange = [0,10]
                        
                        if keyword_set(young) then xrange = [0,20]
                        if keyword_set(intermed) then xrange = [0,6.5]
                        if keyword_set(aged) then xrange = [0,4]
                        ;if keyword_set(LT4) then xrange = [0,6]
                        if keyword_set(nonsmoke) then xrange = [0,1.5]
                        
                        if keyword_set(normalized) then xrange = [0,2]
                        if keyword_set(filter1) and plume eq 1 then xrange = [0,3]
                    ...
                    1:begin
                        ...
                        title = 'Acetaldehyde'
                        xrange = [0,3]
                        if keyword_set(boise) then xrange = [0,2]
                        if keyword_set(CentralValley) then xrange = [0,5]

                        if keyword_set(young) then xrange = [0,15]
                        if keyword_set(intermed) then xrange = [0,3.5]
                        if keyword_set(aged) then xrange = [0,1.5]
                        if keyword_set(nonsmoke) then xrange = [0,0.5]

                        if keyword_set(normalized) then begin
                            xrange = [0,3]
                            if keyword_set(aged) then xrange = [0,4]
                        endif
                        if keyword_set(filter1) and plume eq 1 then xrange = [0,1]
                    ...
                    2:begin
                        ...
                        gvoc_gfas = acet_gc_gfas 
                        gvoc_gfas_bkg_corr = acet_gc_gfas + 0.38
                        gvoc_gfed4 = acet_gc_gfed4
                        gvoc_qfed = acet_gc_qfed
                        gvoc_finn = acet_gc_finn
                        gvoc_threegfas =  acet_gc_threegfas
                        ...
                        title = 'Acetone'
                        xrange = [0,4]
                        if keyword_set(CentralValley) then xrange = [0,8]

                        if keyword_set(young) then xrange = [0,8]
                        if keyword_set(intermed) then xrange = [0,6]
                        if keyword_set(aged) then xrange = [0,5]
                        if keyword_set(nonsmoke) then xrange = [0,2.5]

                        if keyword_set(normalized) then xrange = [0,2]
                        if keyword_set(filter1) and plume eq 1 then xrange = [0,4]
                    ...
                    3:begin
                        ...
                        gvoc_gfas_bkg_corr = c3h8_gc_gfas + 0.035
                        ...
                        xrange = [0,1]
                        if keyword_set(boise) then xrange = [0,2]
                        if keyword_set(CentralValley) then xrange = [0,1.5]
                        yrange = [0,7]
                        
                        if keyword_set(young) then xrange = [0,2.5]
                        if keyword_set(intermed) then xrange = [0,1]
                        if keyword_set(aged) then xrange = [0,2]
                        ;if keyword_set(LT4) then xrange = [0,2.5]
                        if keyword_set(nonsmoke) then xrange = [0,0.1]
                        if keyword_set(normalized) then xrange = [0,3]
                        if keyword_set(filter1) and plume eq 1 then xrange = [0,1.2]
                    ...
                    4:begin
                        ...
                        xrange = [0,0.4]
                        if keyword_set(boise) then xrange = [0,0.4]
                        if keyword_set(CentralValley) then xrange = [0,1]
                        
                        if keyword_set(young) then xrange = [0,1]
                        if keyword_set(intermed) then xrange = [0,0.6]
                        if keyword_set(aged) then xrange = [0,0.4]
                        ;if keyword_set(LT4) then xrange = [0,0.4]
                        if keyword_set(nonsmoke) then xrange = [0,0.5]
                    ...
                    5:begin
                        ...
                        gvoc_gfas = benz_gc_gfas 
                        gvoc_gfas_bkg_corr = benz_gc_gfas + 2.5/1000
                        gvoc_gfed4 = benz_gc_gfed4
                        gvoc_qfed = benz_gc_qfed
                        gvoc_finn = benz_gc_finn
                        gvoc_threegfas = benz_gc_threegfas
                        ...
                        if keyword_set(boise) then xrange = [0,0.6]
                        if keyword_set(CentralValley) then xrange = [0,2]

                        if keyword_set(young) then xrange = [0,0.8]
                        if keyword_set(intermed) then xrange = [0,0.8]
                        if keyword_set(aged) then xrange = [0,0.3]
                        ;if keyword_set(LT4) then xrange = [0,0.5]
                        if keyword_set(nonsmoke) then xrange = [0,0.4]
                        if keyword_set(normalized) then begin
                            xrange = [0,2]
                            if keyword_set(aged) then xrange = [0,3]
                        endif
                    ...
                    6:begin
                        ...
                        if keyword_set(filter1) and plume eq 1 then xrange = [0,0.12]
                    ...
                    7:begin
                        ...
                        gvoc_gfas = c7h8_gc_gfas 
                        gvoc_gfas_bkg_corr = c7h8_gc_gfas + 8.0/1000
                        gvoc_gfed4 = c7h8_gc_gfed4
                        gvoc_qfed = c7h8_gc_qfed
                        gvoc_finn = c7h8_gc_finn
                        gvoc_threegfas = c7h8_gc_threegfas
                        ...
                        title = 'Toluene'
                        xrange = [0,0.25]
                        if keyword_set(CentralValley) then xrange = [0,0.5]
                        
                        if keyword_set(young) then xrange = [0,1]
                        if keyword_set(intermed) then xrange = [0,0.3]
                        if keyword_set(aged) then xrange = [0,0.1]
                        ;if keyword_set(LT4) then xrange = [0,0.3]
                        if keyword_set(nonsmoke) then xrange = [0,0.025]
                        if keyword_set(normalized) then xrange = [0,4]

                        if keyword_set(filter1) and plume eq 1 then xrange = [0,0.1]
                        if keyword_set(filter2_nobb) or keyword_set(filter3_nobb) or keyword_set(filter5_nobb) then xrange = [0,0.08]                            
                    ...
                    8:begin
                        ...
                        gvoc_gfas = c8h10_gc_gfas
                        gvoc_gfas_bkg_corr = c8h10_gc_gfas + 3.5/1000
                        gvoc_gfed4 = c8h10_gc_gfed4
                        gvoc_qfed  = c8h10_gc_qfed
                        gvoc_finn  = c8h10_gc_finn
                        gvoc_threegfas = c8h10_gc_threegfas
                        ...
                        if keyword_set(boise) then xrange = [0,0.1]
                        if keyword_set(CentralValley) then xrange = [0,0.15]
                        
                        if keyword_set(young) then xrange = [0,0.2]
                        if keyword_set(intermed) then xrange = [0,0.04]
                        if keyword_set(aged) then xrange = [0,0.015]
                        ;if keyword_set(LT4) then xrange = [0,0.02]
                        if keyword_set(nonsmoke) then xrange = [0,0.003]
                        if keyword_set(normalized) then begin
                            xrange = [0,4]
                            if keyword_set(aged) then xrange = [0,5]
                        endif
                        if keyword_set(filter1) and plume eq 1 then xrange = [0,0.03]
                    ...
                    10:begin
                        ...
                        gvoc_gfas_bkg_corr = c2h6_gc_gfas
                    ...
                    11:begin
                        ...
                        gvoc_gfas_bkg_corr = alk4_gc_gfas
                    ...
                    12:begin
                        ...
                        gvoc_gfas_bkg_corr = rcho_gc_gfas + 0.02
                    ...
                    13:begin
                        ...
                        gvoc_gfas_bkg_corr = prpe_gc_gfas
                    ...
                    15:begin
                        ...
                        gvoc_finn = hcooh_gc_finn
                        gvoc_gfas = hcooh_gc_gfas
                        gvoc_gfas_bkg_corr = hcooh_gc_gfas + 0.6
                        gvoc_qfed = hcooh_gc_qfed
                        gvoc_threegfas = hcooh_gc_threegfas
                        gvoc_nobb = hcooh_gc_nobb
                    ...
                    16:begin
                        ...
                        gvoc_gfas_bkg_corr = acta_gc_gfas + 0.25
...
;///////////////////////////////////////////////////////////////////////////////////////////
;; for boise data
            if keyword_set(boise) then begin
                ind_lat = where(tmp_lat_obs gt 44.5 or tmp_lat_obs lt 42.5, ct )
                if ct gt 0 then ovoc_ptr[ind_lat] = !values.f_nan
                ;print, ct
                ind_lon = where(tmp_lon_obs lt (-117+360) or tmp_lon_obs gt (-115.5+360), ct)
                if ct gt 0 then ovoc_ptr[ind_lon] = !values.f_nan

            endif
            
;; for central valley            
            if keyword_set(CentralValley) then begin
                ind_lat = where(tmp_lat_obs gt 41.2 or tmp_lat_obs lt 39.2, ct)
                if ct gt 0 then ovoc_ptr[ind_lat] = !values.f_nan
                ind_lon = where(tmp_lon_obs lt (-123.2+360) or tmp_lon_obs gt (-121.2+360), ct)
                if ct gt 0 then ovoc_ptr[ind_lon] = !values.f_nan
            endif            
...
;///////////////////////////////////////////////////////////////////////////////////////////
            if remove_ind[0] ne -1 then begin
                ...
                gvoc_gfas_bkg_corr = gvoc_gfas_bkg_corr[remove_ind]
                ...
                if ~keyword_set(tempplot) and ~keyword_set(presplot) and ~keyword_set(ndplot) then begin
                    koh_voc_obs = koh_voc_obs[remove_ind]
                    koh_voc_mod = koh_voc_mod[remove_ind]
                endif
                ;; addup
                if keyword_set(addup) then begin
                    ovoc_gfed4 = ovoc_gfed4[remove_ind]
                    ovoc_finn = ovoc_finn[remove_ind]
                    ovoc_gfas = ovoc_gfas[remove_ind]
                    ovoc_qfed = ovoc_qfed[remove_ind]
                    ovoc_threegfas = ovoc_threegfas[remove_ind]
...
;+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
;///////////////////////////////////////////////////////////////////////////////////////////
;; plotting for temperature if needed
            if keyword_set(tempplot) then begin
                org_corr,tmp_temperature_obs,tmp_temperature_gc,correlate(tmp_temperature_obs,tmp_temperature_gc),n_elements(tmp_temperature_obs),slope,intercept,slope_se,intercept_se       
                SCATTERPLOT, tmp_temperature_obs, tmp_temperature_gc,_Extra = _Extra,xrange = [250,320], yrange = [250,320],charsize=2,charthick = 8
                oplot,!x.crange,intercept+!x.crange*slope,col=1,line=0, thick=8
                help,tmp_temperature_obs
                
                xyouts,!x.crange(0) + (!x.crange(1)-!x.crange(0))/15,!y.crange(1)*0.98,$
                   /data,col=1,'Slope='+string(strtrim(slope, 1), format='(F5.2,F4.1)') +'+/-' + string(strtrim(slope_se, 1), format='(F5.2,F4.1)'),charsize=3,charthick = 8

                xyouts,!x.crange(0) + (!x.crange(1)-!x.crange(0))/15,!y.crange(1)*0.965,$
                   /data,col=1,'Intercept='+string(strtrim(intercept, 1), format='(F5.1,F4.1)') +'+/-' + string(strtrim(intercept_se, 1), format='(F5.1,F4.1)'),charsize=3,charthick = 8

                xyouts,!x.crange(0) + (!x.crange(1)-!x.crange(0))/15,!y.crange(1)*0.95,$
                   /data,col=1,'R='+ string(strtrim(correlate(tmp_temperature_obs,tmp_temperature_gc), 1),format='(F5.2,F4.1)'),charsize=3,charthick = 8
                continue
            endif
            
            if keyword_set(presplot) then begin
                org_corr,zvar_obs,zvar_gc_gfas,correlate(zvar_obs,zvar_gc_gfas),n_elements(zvar_obs),slope,intercept,slope_se,intercept_se       
                SCATTERPLOT, zvar_obs, zvar_gc_gfas,_Extra = _Extra,xrange = [400,1000], yrange = [400,1000],charsize=2,charthick = 8
                oplot,!x.crange,intercept+!x.crange*slope,col=1,line=0, thick=8
                help,zvar_obs
                ...
                xyouts,!x.crange(0) + (!x.crange(1)-!x.crange(0))/15,!y.crange(1)*0.92,$
                   /data,col=1,'Intercept='+string(strtrim(intercept, 1), format='(F5.1,F4.1)') +'+/-' + string(strtrim(intercept_se, 1), format='(F5.1,F4.1)'),charsize=3,charthick = 8

                xyouts,!x.crange(0) + (!x.crange(1)-!x.crange(0))/15,!y.crange(1)*0.89,$
                   /data,col=1,'R='+ string(strtrim(correlate(zvar_obs,zvar_gc_gfas), 1),format='(F5.2,F4.1)'),charsize=3,charthick = 8
                continue
            endif
            
            if keyword_set(ndplot) then begin
                org_corr,scalefactor_obs/1e25,scalefactor_mod/1e25,correlate(scalefactor_obs,scalefactor_mod),n_elements(scalefactor_obs),slope,intercept,slope_se,intercept_se       
                SCATTERPLOT, scalefactor_obs/1e25, scalefactor_mod/1e25,_Extra = _Extra,xrange = [1,3], yrange = [1,3],charsize=2,charthick = 8
                oplot,!x.crange,intercept+!x.crange*slope,col=1,line=0, thick=8
                ...
                xyouts,!x.crange(0) + (!x.crange(1)-!x.crange(0))/15,!y.crange(1)*0.89,$
                   /data,col=1,'R='+ string(strtrim(correlate(scalefactor_obs,scalefactor_mod), 1),format='(F5.2,F4.1)'),charsize=3,charthick = 8
                continue
            endif
            
            if keyword_set(OHr_output) then begin
                print, 'Observation (PTR)', median(ovoc_ptr*koh_voc_obs)
                print, 'Observation (Others)', median(ovoc_other*koh_voc_obs)
                
                print, 'GFED4', median(gvoc_gfed4*koh_voc_mod)
                print, 'FINN', median(gvoc_finn*koh_voc_mod)
                print, 'GFAS', median(gvoc_gfas*koh_voc_mod)
                print, 'QFED', median(gvoc_qfed*koh_voc_mod)
                print, '3XGFAS', median(gvoc_threegfas*koh_voc_mod)
                print, 'NoBB', median(gvoc_nobb*koh_voc_mod)

            endif
    ;; test
            ;write_csv, 'Temp_obs_gc.csv', tmp_temperature_obs,tmp_temperature_gc
            ;stop
    ...
    ;; just use one altitude
            ...
            if keyword_set(prs) then begin
                if keyword_set(OHrplot) then begin
                    oVert_ptr = bin_vert(Data=ovoc_ptr*koh_voc_obs, ZData = zvar_obs, $
                                        ZEdges=PEdges,/Press)
                    oVert_other = bin_vert(Data=ovoc_other*koh_voc_obs, ZData = zvar_obs, $
                                        ZEdges=PEdges,/Press)
                                        
                                        
                    gVert_gfed4  = bin_vert(Data=gvoc_gfed4*koh_voc_mod, ZData=zvar_gc, $
                                        ZEdges=PEdges,/Press)
                    gVert_finn  = bin_vert(Data=gvoc_finn*koh_voc_mod, ZData=zvar_gc, $
                                        ZEdges=PEdges,/Press)

                    gVert_gfas  = bin_vert(Data=gvoc_gfas*koh_voc_mod, ZData=zvar_gc, $
                                        ZEdges=PEdges,/Press)
                    gVert_gfas_bkg_cor  = bin_vert(Data=gvoc_gfas_bkg_corr*koh_voc_mod, ZData=zvar_gc, $
                                            ZEdges=PEdges,/Press)
                                        
                    gVert_qfed  = bin_vert(Data=gvoc_qfed*koh_voc_mod, ZData=zvar_gc, $
                                        ZEdges=PEdges,/Press)
                    gVert_threegfas = bin_vert(Data=gvoc_threegfas*koh_voc_mod, ZData=zvar_gc, $
                                        ZEdges=PEdges,/Press)
                    gVert_nobb  = bin_vert(Data=gvoc_nobb*koh_voc_mod, ZData=zvar_gc, $
                                        ZEdges=PEdges,/Press)
                endif else begin
                    ;stop
                    ...
                    gVert_gfas  = bin_vert(Data=gvoc_gfas, ZData=zvar_gc, $
                                        ZEdges=PEdges,/Press)
                    gVert_gfas_bkg_corr  = bin_vert(Data=gvoc_gfas_bkg_corr, ZData=zvar_gc, $
                                            ZEdges=PEdges,/Press)
            ...
            if keyword_set(average) then begin
                ...
                gfas_bkg_corr  = gVert_gfas_bkg_corr.DataMean
            ...
            if keyword_set(med) then begin
                ...
                gfas_bkg_corr  = gVert_gfas_bkg_corr.DataMed
...
;if s eq 10 then print, obs
            ...
            ;; gfas_bkg_corr
            gfas_bkg_corr_q10 = gVert_gfas_bkg_corr.DataQ10
            gfas_bkg_corr_q90 = gVert_gfas_bkg_corr.DataQ90
            gfas_bkg_corr_q25 = gVert_gfas_bkg_corr.DataQ25
            gfas_bkg_corr_q75 = gVert_gfas_bkg_corr.DataQ75
            gfas_bkg_corr_numpts = gVert_gfas_bkg_corr.NumPts
            ...
            if ct gt 0 then begin
                ...
                finn  = finn[ind]
                gfas  = gfas[ind]
                gfas_bkg_corr  = gfas_bkg_corr[ind]
                qfed  = qfed[ind]
                ...
                gfas_bkg_corr_q10 = gfas_bkg_corr_q10[ind]
                gfas_bkg_corr_q90 = gfas_bkg_corr_q90[ind]
                gfas_bkg_corr_q25 = gfas_bkg_corr_q25[ind]
                gfas_bkg_corr_q75 = gfas_bkg_corr_q75[ind]
                gfas_bkg_corr_numpts = gfas_bkg_corr_numpts[ind]
            ...
            ;if keyword_set(OHrplot) then begin
                ...
                if not keyword_set(obs_only) then begin
                    ...
                    ;oplot,gfed4_q10,z_gfed4,col=2,thick=5,line=1
                    ;oplot,gfed4_q90,z_gfed4,col=2,thick=5,line=1

                    ;oplot,finn,z_finn,col=3,thick=thick1,line=0 
                    ;oplot,finn_q10,z_finn,col=3,thick=5,line=1 
                    ;oplot,finn_q90,z_finn,col=3,thick=5,line=1

                    ;if s ne 4 then begin
                        oplot,gfas,z_gfas,col=4,thick=thick1,line=0
                        if keyword_set(bkg_corr) then oplot,gfas_bkg_corr,z_gfas,col=8,thick=thick1,line=1

                        ;oplot,gfas_q10,z_gfas,col=4,thick=5,line=1
                        ;oplot,gfas_q90,z_gfas,col=4,thick=5,line=1
                    ;endif
                    ...
                    if (s eq 0 or s eq 1 or s eq 2 or s eq 3 or s eq 4) and not keyword_set(chemistry) then begin
                        ...
                        ;oplot,qfed_q10,z_qfed,col=9,thick=5,line=1
                        ;oplot,qfed_q90,z_qfed,col=9,thick=5,line=1
                    endif
                    ...
                    ;oplot,nobb_q10,z_finn,col=3,thick=5,line=1 
                    ;oplot,nobb_q90,z_finn,col=3,thick=5,line=1
                endif
            ...
            ;; calcualte uncertainty across 3 inventoires
            ;if dd eq 0 then begin
            ;    uncertainty = []
            ;    for n=0, n_elements(gfed4)-1 do begin
            ;        if title eq 'Formaldehyde' or title eq 'Acetaldehyde' or title eq 'Acetone' or title eq 'Propane' or title eq 'MEK' or title eq 'CO' then $
            ;        uncertainty = [uncertainty, (max([gfed4[n], gfas[n], qfed[n]]) - min([gfed4[n], gfas[n], qfed[n]]))/max([gfed4[n], gfas[n], qfed[n]])]
            ;        
            ;        if title eq 'Benzene' or title eq 'Toluene' or title eq 'Xylene' then $
            ;        uncertainty = [uncertainty, (max([gfed4[n], gfas[n]]) - min([gfed4[n], gfas[n]]))/max([gfed4[n], gfas[n]])]
            ;    endfor
